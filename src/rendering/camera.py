...
def project_points(xyz: torch.Tensor, camera) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    device = xyz.device
    R = (
        torch.from_numpy(camera.R).to(device)
        if hasattr(camera, "R")
        else torch.eye(3, device=device)
    )
    T = (
        torch.from_numpy(camera.T).to(device)
        if hasattr(camera, "T")
    ...
    fx = getattr(camera, "fx", 0.5 * camera.width / math.tan(camera.FovX * 0.5))
    fy = getattr(camera, "fy", 0.5 * camera.height / math.tan(camera.FovY * 0.5))
    cx = getattr(camera, "cx", camera.width * 0.5)
    cy = getattr(camera, "cy", camera.height * 0.5)

    x = (xyz_cam[:, 0] / depth) * fx + cx
    y = (xyz_cam[:, 1] / depth) * fy + cy
